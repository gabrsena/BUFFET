...
const Hero: React.FC<Props> = ({ scrollY }) => {
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center bg-[#0f172a] overflow-hidden px-6 py-20">
      ...
      ></div>
      <div 
        className="absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] bg-brand-pink/30 rounded-full blur-[120px] animate-pulse-glow" 
      ...
      {extraEmojis.map((e, i) => (
        <div 
          ...
          style={{ 
            left: e.left, 
            top: e.top, 
            transform: `translateY(${scrollY * e.speed}px)`,
      ...
      <div className="container mx-auto relative z-10 text-center max-w-5xl">
        <div className="inline-block glass-card px-8 py-3 rounded-full text-brand-yellow font-bold mb-14 border border-white/20 shadow-[0_0_20px_rgba(250,204,21,0.3)] animate-bounce-soft uppercase tracking-[0.3em] text-xs">
        ...
        <p className="text-xl md:text-3xl text-purple-100 mb-16 font-medium max-w-3xl mx-auto leading-relaxed drop-shadow-md">
